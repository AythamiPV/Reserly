...
export class CalendarComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (
      ...
      console.warn(
        '⚠️ Elementos del calendario no encontrados en ngAfterViewInit.'
      );
      return;
    }
  ...
  renderCalendar() {
    if (!this.monthYear || !this.calendarDays || !this.dateInput) {
      console.warn(
        '⚠️ Elementos del calendario no encontrados en renderCalendar.'
      );
      return;
    }
    ...
    this.monthYear.textContent = firstDay.toLocaleString('en-US', {
      month: 'long',
      year: 'numeric',
    });
    ...
    for (
      let i = prevLastDay.getDate() - startDay + 1;
    ...
    for (let i = 1; i <= lastDay.getDate(); i++) {
      ...
      this.renderer.listen(dateElement, 'click', () => {
        this.calendarDays
          ...
          .forEach((d) => this.renderer.removeClass(d, 'selected'));
    ...
    if (
      ...
      if (
        ...
        Array.from(this.calendarDays.children)
          .filter(
            (day) =>
              day.textContent === todayDate.toString() &&
        ...
        const todayElement = Array.from(this.calendarDays.children).find(
          (day) =>
            day.textContent === todayDate.toString() &&
            !day.classList.contains('inactive')
        );
